	...
	_indexUI.prototype = {
		...
		*/
		initAjaxGetter: function(){
			_ajaxGetter.init(INDEX + "/Media/Search/page", 1, 20, false);
		},
		...
		*/
		getCateItems: function(){
			// 获取分类信息
			_ajaxGetter.init(INDEX + "/Media/Search/cate", 1, 20, false)
			.getItems({
		...
		*/
		changeMovieGettingRule: function (obj){
			...
			if (rule === 'all'){
				...
				_ajaxGetter.init(INDEX + "/Media/Shelf/page", 1, 20, false);
			}else{
				_ajaxGetter.init(INDEX + "/Media/Search/rank", 1, 20, false);
				extraFormData = {
		...
		*/
		setSearchRule: function (){
			...
			_ajaxGetter.init(INDEX + "/Media/Search/search", 1, 20, false);
			extraFormData = {
				'key' : $('#key').val(),
			}
			// 清屏
		...
		purchaseItem: function (obj){
			...
			$.ajax({
				...
				success : function(result) {
					if (result.success){
						$this.play(result.data.src, cover);
					}else{
						console.log(result);
					}
				},
